      ...
      try {
        ...
        const firebaseData = querySnapshot.docs.map(doc => {
          ...
          return {
            ...
            approvedShares: d["Approved List of Shares"] ?? d["Approved List of MF"] ?? "—",
            tenure: d["Tenure"] ?? "—",
    ...
    <div className="bg-gradient-to-b from-pink-50 via-white to-teal-50 min-h-screen text-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-6 py-12 space-y-16">
        ...
        <div className="bg-white shadow-lg rounded-2xl p-8">
          <p className="text-pink-600 italic text-center text-sm">Snapshot of what lenders offer</p>
          <h2 className="text-3xl font-bold text-center mb-8">Funding Related Details</h2>
            ...
            <table className="w-full border-collapse">
                    ...
                    <th key={h.key} className="px-4 py-3 border-b text-sm font-semibold">
                      <div className="flex items-center gap-1">
                        ...
                        {sortFieldFunding === h.key && (
                          <span className="text-xs">{sortOrderFunding === 'asc' ? '▲' : '▼'}</span>
                        )}
                      </div>
                    </th>
                  ))}
              ...
              <tbody>
                {sortedFundingData.map((row, idx) => (
                  <tr key={row.id} className={`hover:bg-gray-50 ${idx % 2 === 0 ? "bg-white" : "bg-gray-50/60"}`}>
                    ...
                    <td className="px-4 py-3 border-b text-teal-600 font-medium">{row.minRate}</td>
                    <td className="px-4 py-3 border-b text-pink-600 font-medium">{row.maxRate}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {/* Major Cost Section */}
        <div className="bg-white shadow-lg rounded-2xl p-8">
          <p className="text-teal-600 italic text-center text-sm">Understand the true cost of borrowing</p>
                  ...
                  {[
                    ...
                    <th key={h.key} className="px-4 py-3 border-b text-sm font-semibold">
                      <div className="flex items-center gap-1">
                        ...
                        {sortFieldCost === h.key && (
                          <span className="text-xs">{sortOrderCost === 'asc' ? '▲' : '▼'}</span>
                        )}
                      </div>
                    </th>
                  ))}
              ...
              <tbody>
                {sortedCostData.map((row, idx) => (
                  <tr key={row.id} className={`hover:bg-gray-50 ${idx % 2 === 0 ? "bg-white" : "bg-gray-50/60"}`}>
                    ...
                    <td className="px-4 py-3 border-b">{row.prepaymentCharges}</td>
